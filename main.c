 ...
 * @retval 1 Error
 */
static int mutt_init(struct ConfigSet *cs, struct Buffer *dlevel, struct Buffer *dfile,
                     bool skip_sys_rc, struct StringArray *commands)
{
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  int rc2 = mutt_init(cs, &cli->shared.log_level, &cli->shared.log_file,
                      cli->shared.disable_system, &cli->shared.commands);
  if (rc2 != 0)
    goto main_curses;
  ...
  else
  {
    ...
    else if (cli->gui.start_nntp || cli->gui.start_browser)
    {
      if (cli->gui.start_nntp)
      {
    ...
    if (cli->gui.start_any_mail || cli->gui.start_new_mail)
    {
    ...
    if (!mx_mbox_open(m, (cli->gui.read_only || c_read_only) ? MUTT_READONLY : MUTT_OPEN_NO_FLAGS))
    {
      if (m->account)
