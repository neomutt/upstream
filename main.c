...
static int mutt_init(struct ConfigSet *cs, struct Buffer *dlevel,
                     struct Buffer *dfile, bool skip_sys_rc, struct StringArray *commands)
{
 ...
 * @param ch Command Line Options
 * @retval true Success
 */
static bool show_help(struct CliHelp *ch)
{
  if (!ch->is_set)
    return true;
 ...
 * @param cl Command line Options
 * @param cs Config Set
 * @retval true Succes
 */
static bool init_logging(struct CliLogging *cl, struct ConfigSet *cs)
{
  if (!cl->is_set)
    return true;

  if (!buf_is_empty(&cl->log_file))
    config_str_set_initial(cs, "debug_file", buf_string(&cl->log_file));

  if (!buf_is_empty(&cl->log_level))
  {
    const char *dlevel = buf_string(&cl->log_level);
    short num = 0;
...
int main(int argc, char *argv[], char *envp[])
{
  ...
  if (!init_logging(&cli->logging, cs))
    goto main_exit;
  ...
  int rc2 = mutt_init(cs, &cli->logging.log_level, &cli->logging.log_file,
                      flags & MUTT_CLI_NOSYSRC, &cli->gui.commands);
  if (rc2 != 0)
    goto main_curses;
  ...
  init_nntp(&cli->gui.nntp_server, cs);

  /* Initialize crypto backends.  */
  crypt_init();

  if (!buf_is_empty(&cli->gui.mbox_type) &&
      !config_str_set_initial(cs, "mbox_type", buf_string(&cli->gui.mbox_type)))
  {
    goto main_curses;
  }
  ...
  {
    struct Buffer *folder = &cli->gui.folder;
