...
static void serialise_gui(struct CliGui *gui, struct Buffer *res)
{
  if (!gui->is_set)
    return;
  ...
  serialise_bool(gui->read_only, res);
  serialise_bool(gui->start_postponed, res);
  serialise_bool(gui->start_browser, res);
  serialise_bool(gui->start_nntp, res);
  serialise_bool(gui->start_new_mail, res);
  serialise_bool(gui->start_any_mail, res);

  serialise_buffer(&gui->folder, res);
  serialise_buffer(&gui->nntp_server, res);
